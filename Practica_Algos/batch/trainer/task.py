...
def generate_label_encoder(train_df):
    ...
    encoder.fit(train_df.troll_or_not.tolist())
    return encoder
...
def train_and_evaluate(
    work_dir, train_df, eval_df, batch_size=64, epochs=1, steps=100
):
    ...
    y_train = label_encoder.transform(train_df.troll_or_not.tolist())
    logging.info("---- Encoding eval target ----")
    y_eval = label_encoder.transform(eval_df.troll_or_not.tolist())
...
if __name__ == "__main__":
    ...
    train_df = pd.concat(
        [
            pd.read_csv(
                f,
                names=["text", "troll_or_not"],
                dtype={"text": "string", "troll_or_not": "string"},
            )
